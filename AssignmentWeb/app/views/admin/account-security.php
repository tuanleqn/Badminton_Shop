...
<div class="page-heading">
    ...
    <?php if ($session->hasFlash('success') || $session->hasFlash('error') || $session->hasFlash('message')): ?>
    <div class="toast-container position-fixed top-0 end-0 p-3">
        <div class="toast show" role="alert" aria-live="assertive" aria-atomic="true">
            <div class="toast-header <?php echo $session->hasFlash('error') ? 'text-bg-danger' : ($session->hasFlash('success') ? 'text-bg-success' : 'text-bg-primary'); ?>">
                <svg class="bd-placeholder-img rounded me-2" width="20" height="20" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" preserveAspectRatio="xMidYMid slice" focusable="false">
                    <rect width="100%" height="100%" fill="#FFFFFF"></rect>
                </svg>
                <strong class="me-auto text-white">
                    <?php 
                    if ($session->hasFlash('error')) echo 'Error';
                    else if ($session->hasFlash('success')) echo 'Success';
            ...
            <div class="toast-body">
                <?php 
                if ($session->hasFlash('error')) echo $session->getFlash('error');
                else if ($session->hasFlash('success')) echo $session->getFlash('success');
                else echo $session->getFlash('message');
                ?>
            </div>
        </div>
    </div>
        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="<?php echo URL::to('public/admin/adminUpdatePassword'); ?>" method="POST">
