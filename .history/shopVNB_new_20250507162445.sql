...
ALTER TABLE `order`
ADD COLUMN `status` ENUM('pending', 'shipped', 'delivered', 'cancelled') DEFAULT 'pending' AFTER `totalPayment`;
...
ADD COLUMN `status` ENUM('pending', 'shipped', 'delivered', 'cancelled') DEFAULT 'pending' AFTER `image`;
...
ALTER TABLE `product` DROP FOREIGN KEY `fk_product_branch`;

-- Rename the column from branchId to brandId
ALTER TABLE `product` DROP FOREIGN KEY `actual_foreign_key_name`;

-- Rename the column from branchId to brandId
ALTER TABLE `product` CHANGE `branchId` `brandId` INT NOT NULL;

-- Add a new foreign key constraint referencing the brand table
ALTER TABLE `product` ADD CONSTRAINT `fk_product_brand` FOREIGN KEY (`brandId`) REFERENCES `brand`(`id`);
