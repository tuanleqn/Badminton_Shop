...
function processOrder() {
    ...
    if (!fullNameRegex.test(fullNameElement.value.trim()) || fullNameElement.value.trim().length < 3 || fullNameElement.value.trim().length > 50) {
        document.getElementById('fullNameError').textContent = 'Họ và tên không được để trống và phải từ 3 đến 50 ký tự và chỉ chứa chữ cái.';
        isValid = false;
    }
